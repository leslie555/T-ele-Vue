<template>
        <div>
            <div style="margin-top:15px;">
                <span class="fitmentItem">填写采购单</span>
                <el-button @click="addItem" size="small" plain type="primary">添加项目</el-button>
            </div>
            <el-table
                :data="tableData"
                border
                ...
                <el-table-column
                label="供货商"
                prop="SupplyCommodityCategoryID"
                class="table-normal"
                min-width="150">
                    <template slot-scope="scope">
                        ...
                        <el-cascader
                            :disabled="scope.row.ids"
                            v-model="scope.row.arrId"
                            :options="ShowAllSupplierCategory"
                ...
                <el-table-column
                label="项目名称"
                prop="RenovationApplyConfigueID"
                min-width="150">
                    <template slot-scope="scope">
                ...
                min-width="120">
                    <template slot-scope="scope">
                        ...
                        <el-input v-else size="small" type="number" @input="getCurrentRow(scope.row.Number, scope.$index, scope.row)"  v-model="scope.row.Number"></el-input>
                    </template>
                </el-table-column>
                <el-table-column
...
<script>
    // import { ShowPurchasingOrderDetails, UpdatePurchasingOrderDetails, DelPurchasingOrderDetails, AddPurchasingOrderDetails } from '@/api/purchase'
    import { ShowPurchasingOrderDetails, AddPurchasingOrderDetails, UpdatePurchasingOrderDetails, ShowSupplyCommodityProject } from '@/api/purchase'
  export default {
    name: 'ItemFitment',
    ...
    methods: {
        ...
        SaveClick(row, index) {
            if (row.SupplyCommodityCategoryID === '' || row.SupplyCommodityConfigueID === '') {
            ...
            if (row.KeyID || row.KeyID === 0) {
                UpdatePurchasingOrderDetails(row).then(({ Data, BusCode, Msg }) => {
                ...
                this.$confirm('确定保存这条数据？, 是否继续？', '提示', {
                    ...
                    AddPurchasingOrderDetails(row).then(({ Data, BusCode, Msg }) => {
                        this.tableData[index].ids = !this.tableData[index].ids
                        ...
                        this.savaTableList[index] = this.$deepCopy(this.tableData[index])
                        // 监听长度判断提交
                        // this.$emit('changeLength', { length: this.savaTableList.length, index: 1 })
                    }).catch(() => {
                    })
                })
            }
        ...
        getDetailData(KeyID) {
          ...
          ShowPurchasingOrderDetails({
              ...
              this.tableData.forEach(val => {
                ...
                val.arrId = [val.SupplyCommodityCategoryID, val.SupplyCommodityConfigueID]
              })
              this.listLoading = false
              if (this.tableData.length !== 0) {
                this.form.purchaseRemark = this.tableData[0].CGBZ
              }
        ...
        projectChange(data, index, row) {
            row.SupplyCommodityCategoryID = data[0]
            row.SupplyCommodityConfigueID = data[1]
        },
        // 下拉框的接口
        async selectApi() {
            const [type1] = await Promise.all([ShowSupplyCommodityProject()])
            console.log(type1.Data.Distributories, type1.Data.categories)
            const filterProject = type1.Data.Distributories.map(v => ({
                label: v.Distributor,
                value: v.KeyID,
                pid: v.CategoryID
            }))
            this.ShowAllSupplierCategory = type1.Data.categories.map(v => ({
                label: v.CategoryName,
                value: v.KeyID,
                children: filterProject.filter(val => val.pid === v.KeyID)
            }))
        }
    },
    created() {
    ...
    watch: {
        ...
        tableData: {
            handler: function(oldV, newV) {
                this.$emit('changeData', { data: this.tableData, oldData: this.deepTableData, index: 1 })
            },
            deep: true
        }
    }
