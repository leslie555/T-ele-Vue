  ...
  methods: {
    open(val, name) {
      ...
      if (val.num === 1) {
        ...
        if (this.bill === 1) {
        ...
        } else if (this.bill === 10) {
          this.nameJudge = `指派${name}人员`
          this.rules = {
            MaintainEmpID: [
              {
                required: true,
          ...
          this.nameJudge = '指派搬家人员'
          this.rules = {
            MaintainEmpID: [
              { required: true, message: '请指派搬家人员', trigger: 'change' }
            ]
          }
        }
      } else if (val.num === 2) {
    ...
    handleSelect(val) {
      ...
      this.formInline.ShareName = val.UserName
    },
    //   模糊查询
    querySearch(queryString, cb) {
      ...
      var results = queryString
        ? designate.filter(this.createFilter(queryString))
        : designate
    ...
    createFilter(queryString) {
      return designate => {
        return (
          designate.value.toLowerCase().indexOf(queryString.toLowerCase()) === 0
        )
      }
    },
    submitForm(formName) {
      if (this.num === 1) {
        this.$refs[formName].validate(valid => {
          if (valid) {
            ...
            if (this.bill === 1) {
              ...
              // this.showDialog = false
              this.showDialog = false
              console.log(this.formInline, '123')
              this.$emit('refresh', this.formInline, this.ListData, 10)
            } else {
              this.showDialog = false
        ...
        postData.MaintainImg = this.$DiffArrFn(arr, this.form.ImgList, [
          'KeyID'
        ])
        if (this.bill === 1) {
          const obj = {
          ...
          console.log(
            this.form.ImgList,
            postData.MaintainIDSum,
            postData.MaintainImg,
            obj
          )
          EditMaintain(obj).then(({ Data, BusCode, Msg }) => {
            this.$emit('refresh', 'Stock')
