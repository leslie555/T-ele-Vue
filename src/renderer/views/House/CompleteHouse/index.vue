  ...
  <div class="app-container">
    <div class="panel" v-loading="CompleteLoading">
      <steps-box :list="stepList" ref="steps">
        ...
        <template slot="3">
          <p class="panel-title"><span style="color: red;font-size: 12px">*</span>房源图片</p>
...
<script>
  ...
  import { houseInfoListByID, HousingSumbit, perfectingHousingSumbitEdit } from '@/api/house'
  ...
  export default {
    ...
    methods: {
      ...
      HouseImgNext() {
        // 验证图片必须传
        let flag = false
        this.HouseInfoImg.forEach(x => {
          if (x.length === 0) {
            flag = true
          }
        })
        if (flag) {
          this.$alert('公共区域和房间图片都必须上传才能提交', '温馨提示', {
