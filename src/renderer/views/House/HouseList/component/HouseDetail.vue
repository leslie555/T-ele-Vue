<template>
  <div>
    <transition-group name="HouseFade">
      ...
      >
        ...
        <el-row class="DialogContentBox">
          <el-form size="mini" :model="dialogFormData">
            ...
            <!-- 租约信息 -->
            <el-row class="DialogContent">
              <el-col :span="16" class="dialogCol" style="min-height: 200px;">
                <el-form-item class="ContentTitle">
                  <span>租约信息</span>
                </el-form-item>
                <el-row v-if="dialogFormData.tenantInfoDetail.KeyID > 0">
                  ...
                  <el-form-item label="是否续约:">
                    <span>{{ IsContractExtension === 0?'待确定':'不续约' }}</span>
                  </el-form-item>
              ...
              >
                ...
                <el-button type="warning" plain size="small" class="dialogBtn" @click="OpenRev" v-setbtn:AddSubscribe>
                  添加预约
                </el-button>
                <el-button
                  type="warning"
      ...
      <house-renew ref="HouseRenew" key="35" @handleSuccess="HouseRenewSuccess"></house-renew>
    </transition-group>
  </div>
</template>
<script>
  ...
  import HouseRenew from './component/HouseRenew'
  ...
  import { editHouseInfoEmp, FindFitment, FindHouseWhetherRentOut, selectMyHouseEmp } from '@/api/house'
  ...
  export default ({
    ...
    watch: {
      ...
      'isShow': {
        handler() {
          if (this.isShow && this.dialogFormData.tenantInfoDetail.KeyID > 0) {
            FindHouseWhetherRentOut(
              { HouseID: this.dialogFormData.KeyID }
            ).then(({ Data }) => {
              this.IsContractExtension = Data.IsContractExtension
            })
          }
        }
      }
    ...
    methods: {
      ...
      ToggleRenew() {
        this.$refs.HouseRenew.open({
          status: this.IsContractExtension,
          type: 2,
          HouseID: this.dialogFormData.KeyID
        })
      },
      HouseRenewSuccess(data) {
        this.IsContractExtension = data.status
      },
      ChangeRemark(msg) {
